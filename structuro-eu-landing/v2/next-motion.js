...
(function () {
  ...
  function initNav() {
    ...
    function sync() {
      ...
      var shouldHide = !menuOpen && goingDown && y > 80;
      if (goingUp || y < 40 || menuOpen) shouldHide = false;
  ...
  function initReveal() {
    /* Content is zichtbaar zonder scroll-reveal (COGA / WCAG 2.2.2). */
  ...
  function initStickyCta() {
    ...
    function update() {
      // Geen dubbele CTA: verberg de sticky balk zodra de slot-CTA in beeld is.
      setVisible(mobile.matches && !heroInView && !slotInView);
    }
    if ("IntersectionObserver" in window) {
      var io = new IntersectionObserver(
        function (entries) {
      ...
      io.observe(hero);
      if (slot) {
        var ioSlot = new IntersectionObserver(
          function (entries) {
            var entry = entries[0];
            if (!entry) return;
