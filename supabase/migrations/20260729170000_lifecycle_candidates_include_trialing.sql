-- Lifecycle candidates: Stripe card-trial (trialing) moet in de view blijven.
-- Live DB had dit al (alleen active uitgesloten); repo-migraties sloten trialing uit.
-- Ook subscription_current_period_end is nodig voor S4 timing op period_end.
...
COMMENT ON VIEW public.lifecycle_candidates_v1 IS
  'Lifecycle candidates (geen active). Trialling blijft erin voor 7d card-trial drip. Audience off/test/v2/all filtert in app.';
