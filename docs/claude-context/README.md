...
Webapp voor volwassenen met ADHD-achtige kenmerken: **dagstart/check-in**, **taken met prioriteiten en energiezones**, **focusmodus**, **herinneringen/notificaties**, **instellingen** en **onboarding**.
...
| `/agenda`             | `app/agenda/page.tsx`       | Redirect naar `/` (geen apart agenda-scherm)                                         |
...
**API** (selectie): `app/api/tasks/*`, `app/api/stripe/*`, enz. Gebruikt voor server-side operaties waar nodig.
...
- Optionele **cyclusfase** (menstruatiecyclus, geen medisch model): zie **[CYCLE_TRACKING.md](./CYCLE_TRACKING.md)**. Wordt bij consent op `profiles` bewaard en als `cycle_phase` bij de dagstart-check-in meegeslagen; suggestieteksten staan achter `NEXT_PUBLIC_STRUCTURO_CYCLE_SUGGESTIONS`.
...
## 11. Geen gamification-productlaag

Er is **geen** apart scherm, XP-systeem of `lib/xp.ts` meer in de app. In Supabase kan de tabel `gamification_data` uit oudere schema’s nog bestaan; de client gebruikt die niet meer.

---

## 12. Focus en overige domeinlib

- `app/focus/page.tsx` + timer, `parkFocusTask`, microstappen, analytics-events.
