...
export default function TestDataPageClient() {
  ...
  const addTestTasks = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("tasks").insert(testTasks).select();
      ...
      console.error("Error:", error);
      toast(`Fout: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <ul style={{ fontSize: 14, color: "#374151", marginBottom: 24, paddingLeft: 20, lineHeight: 1.8 }}>
          <li>
            3 taken met <strong>lage energie</strong> (10-20 min)
          </li>
          <li>
            4 taken met <strong>normale energie</strong> (30-90 min)
          </li>
          <li>
            3 taken met <strong>hoge energie</strong> (120-180 min)
          </li>
        </ul>
        <button
          type="button"
