...
describe("upsertCheckInToSupabase onAllTasksDropped", () => {
  it("gooit standaard als alle gekozen taken ghost zijn (interactieve save)", async () => {
    const { upsertCheckInToSupabase } = await import("./checkinsDb");
    taskRows = [];
    await expect(
      upsertCheckInToSupabase(USER, DATE, {
        energy_level: "high",
        top3_task_ids: ["ghost-a"],
      })
    ).rejects.toThrow(/Geen van je gekozen taken/);
  });

  it("cleart stil als onAllTasksDropped=clear", async () => {
    const { upsertCheckInToSupabase } = await import("./checkinsDb");
    taskRows = [];
    const saved = await upsertCheckInToSupabase(
      USER,
      DATE,
      { energy_level: "high", top3_task_ids: ["ghost-a"] },
      { onAllTasksDropped: "clear" }
    );
    expect(saved.top3_task_ids).toBeNull();
  });
});
