...
function ObEnergyCardPicker({
  ...
  return (
    <div
      className={`grid grid-cols-3 gap-2 ${staggerReveal ? "ob-energy-picker-stagger" : ""}`}
    >
...
const FIRST_DAY_REVEAL_SUB = 3;
const FIRST_DAY_REVEAL_CYCLE_HINT = 4;
const FIRST_DAY_REVEAL_CYCLE_CARD = 5;
const FIRST_DAY_REVEAL_CYCLE_FOOTNOTE = 6;
const FIRST_DAY_REVEAL_ENERGY = 7;
const FIRST_DAY_REVEAL_AFTER_GREETING_MS = 650;
const FIRST_DAY_REVEAL_ANIM_MS = 480;
/** Welkom start 30% vóór einde slide-in. */
const FIRST_DAY_GREETING_START_MS = Math.round(SLIDE_MS * 0.7);
/** Laten we beginnen: rustig infaden, tussen snel (480ms) en traag (>2s). */
...
const FIRST_DAY_CYCLE_FOOTNOTE_FADE_MS = 850;
const FIRST_DAY_REVEAL_AFTER_FOOTNOTE_MS = 450;
const FIRST_DAY_ENERGY_ANIM_MS = 680;
const FIRST_DAY_ENERGY_STAGGER_MS = 480;
/** Cyclus ingevuld → persoonlijk welkom: crossfade i.p.v. horizontale slide. */
const CYCLE_WELCOME_FADE_OUT_MS = 520;
const CYCLE_WELCOME_FADE_IN_MS = 640;
...
type OnboardingFlowContentProps = {
  isLocalMode: boolean;
  user: User | null;
};

export default function OnboardingFlowContent({
  isLocalMode,
  user,
  ...
  const [firstDayCycleRingSpinActive, setFirstDayCycleRingSpinActive] = useState(false);
  const firstDayCycleHintText = useMemo(
  ...
  /** Cyclus-inhoud mounted houden tijdens slide-out (skip), anders lege flits. */
  const [cycleSlideContentMounted, setCycleSlideContentMounted] = useState(false);
  /** idle | out = cyclus wegfaden; in = welkom infaden (na submit cyclus). */
  const [cycleWelcomeCrossfade, setCycleWelcomeCrossfade] = useState<
    "idle" | "out" | "in"
  >("idle");
  const cycleWelcomeCrossfadeTimersRef = useRef<number[]>([]);
  ...
  /** Eerste dagstart: korte adempauze na energie, dan brugzin, daarna taakkaart. */
  useEffect(() => {
    if (step !== FIRST_DAY_SLIDE_INDEX) {
      setFirstDayBridgeVisible(false);
    ...
    const rm = reduceMotionRef.current;
    if (rm) {
      setFirstDayBridgeVisible(true);
      setFirstDayTaskPhaseVisible(true);
      return;
    }
    setFirstDayBridgeVisible(false);
    setFirstDayTaskPhaseVisible(false);
    const bridgeTimer = window.setTimeout(() => setFirstDayBridgeVisible(true), 480);
    const taskTimer = window.setTimeout(() => setFirstDayTaskPhaseVisible(true), 1350);
    return () => {
  ...
  useEffect(() => {
    if (step === CYCLE_OPT_IN_SLIDE_INDEX) {
      setCycleSlideContentMounted(true);
      return;
    }
    if (step > CYCLE_OPT_IN_SLIDE_INDEX) {
      const delay = cycleWelcomeCrossfade === "idle" ? SLIDE_MS : 0;
      const timer = window.setTimeout(() => setCycleSlideContentMounted(false), delay);
      return () => window.clearTimeout(timer);
    }
    setCycleSlideContentMounted(false);
  }, [step, cycleWelcomeCrossfade]);

  useEffect(() => {
    if (cycleSlideContentMounted) return;
    setCycleStage("intro");
    setCycleInfoPulse(false);
  }, [cycleSlideContentMounted]);
  ...
  useEffect(() => {
    ...
    const timelineId = ++firstDayRevealTimelineIdRef.current;
    ...
    const greetingStart = FIRST_DAY_GREETING_START_MS;
    const introStart = greetingStart + FIRST_DAY_REVEAL_AFTER_GREETING_MS;
    const afterIntro =
      introStart + FIRST_DAY_INTRO_FADE_MS + FIRST_DAY_REVEAL_AFTER_INTRO_MS;
    schedule(greetingStart, () => setFirstDayRevealPhase(FIRST_DAY_REVEAL_GREETING));
    ...
    if (hasCycle) {
      ...
      schedule(cardStart, () => {
        ...
        setFirstDayCyclePart(1);
        setFirstDayCycleRingSpinActive(true);
      });
      schedule(cardStart + FIRST_DAY_CYCLE_PART_STEP_MS, () => setFirstDayCyclePart(2));
      schedule(cardStart + FIRST_DAY_CYCLE_PART_STEP_MS * 2, () => setFirstDayCyclePart(3));
      schedule(cardStart + FIRST_DAY_CYCLE_PART_STEP_MS * 3, () => setFirstDayCyclePart(4));
      const footnoteStart = cardStart + FIRST_DAY_CYCLE_RING_SPIN_MS;
      schedule(footnoteStart, () => setFirstDayRevealPhase(FIRST_DAY_REVEAL_CYCLE_FOOTNOTE));
      schedule(
        footnoteStart + FIRST_DAY_CYCLE_FOOTNOTE_FADE_MS + FIRST_DAY_REVEAL_AFTER_FOOTNOTE_MS,
        () => setFirstDayRevealPhase(FIRST_DAY_REVEAL_ENERGY)
      );
    } else {
      schedule(afterIntro, () => setFirstDayRevealPhase(FIRST_DAY_REVEAL_SUB));
      schedule(afterIntro + FIRST_DAY_REVEAL_AFTER_SUB_MS, () =>
        setFirstDayRevealPhase(FIRST_DAY_REVEAL_ENERGY)
      );
    }
    return () => {
      firstDayRevealTimelineIdRef.current += 1;
          ...
          >
            ...
            >
              <div className="flex min-h-full w-full flex-col justify-center px-5 py-8 md:px-4">
                <div className="mx-auto flex w-full max-w-[520px] min-h-0 flex-1 flex-col justify-center">
                  <div
                    className={`flex flex-col items-center text-center transition-opacity ease-out ${
                      cycleWelcomeCrossfade === "out"
                        ? "pointer-events-none opacity-0 duration-[520ms]"
            ...
            >
              <style>{`
                ...
                .ob-first-day-reveal-in {
                  animation: ob-first-day-reveal-in ${FIRST_DAY_REVEAL_ANIM_MS}ms cubic-bezier(0.22, 1, 0.36, 1) both;
                }
                .ob-first-day-intro-in {
                  animation: ob-first-day-reveal-in ${FIRST_DAY_INTRO_FADE_MS}ms cubic-bezier(0.12, 1, 0.28, 1) both;
                }
                @keyframes ob-cycle-ring-spin-once {
                  from { transform: rotate(0deg); }
                  to { transform: rotate(360deg); }
                }
                .ob-cycle-ring-spin-once {
                  transform-origin: center;
                  animation: ob-cycle-ring-spin-once ${FIRST_DAY_CYCLE_RING_SPIN_MS}ms cubic-bezier(0.28, 0.85, 0.32, 1) both;
                }
                .ob-cycle-part-in {
                  animation: ob-first-day-reveal-in ${FIRST_DAY_CYCLE_PART_FADE_MS}ms cubic-bezier(0.12, 1, 0.28, 1) both;
                }
                .ob-cycle-card-shell-in {
                  animation: ob-first-day-reveal-in 720ms cubic-bezier(0.12, 1, 0.28, 1) both;
                }
                .ob-cycle-footnote-fade-in {
                  animation: ob-first-day-reveal-in ${FIRST_DAY_CYCLE_FOOTNOTE_FADE_MS}ms cubic-bezier(0.22, 1, 0.36, 1) both;
                }
                .ob-energy-picker-stagger > button {
                  opacity: 0;
                  animation: ob-first-day-reveal-in ${FIRST_DAY_ENERGY_ANIM_MS}ms cubic-bezier(0.22, 1, 0.36, 1) both;
                }
                .ob-energy-picker-stagger > button:nth-child(1) { animation-delay: 0ms; }
                .ob-energy-picker-stagger > button:nth-child(2) { animation-delay: ${FIRST_DAY_ENERGY_STAGGER_MS}ms; }
                .ob-energy-picker-stagger > button:nth-child(3) { animation-delay: ${FIRST_DAY_ENERGY_STAGGER_MS * 2}ms; }
                @keyframes ob-first-day-bridge-in {
                  from { opacity: 0; transform: translateY(6px); }
                  to { opacity: 1; transform: translateY(0); }
                }
                .ob-first-day-bridge-in {
                  animation: ob-first-day-bridge-in 680ms cubic-bezier(0.22, 1, 0.36, 1) both;
                }
                ...
                .ob-first-day-task-in {
                  animation: ob-first-day-task-in 760ms cubic-bezier(0.22, 1, 0.36, 1) both;
                }
                ...
                <div className="flex min-h-full w-full flex-col px-5 py-8 pb-28 md:px-4">
                  <div className="mx-auto flex w-full max-w-[520px] min-h-0 flex-1 flex-col justify-center">
                    <div className="flex flex-col items-center text-center">
                      ...
                      {firstDayRevealPhase >= FIRST_DAY_REVEAL_CYCLE_CARD &&
                      cyclePreviewProfile ? (
                        <div
                          className={`mt-5 w-full max-w-md text-left ${
                            firstDayCycleRingSpinActive ? "ob-cycle-card-shell-in" : ""
                          }`}
                        >
                          <CycleEnergyContext
                            consentOn
                            ...
                            ringIntroSpin={firstDayCycleRingSpinActive}
                            onboardingRevealPart={firstDayCyclePart}
                            onboardingFootnoteVisible={
                              firstDayRevealPhase >= FIRST_DAY_REVEAL_CYCLE_FOOTNOTE
                            }
                            onboardingFootnoteFadeIn={
                              firstDayRevealPhase >= FIRST_DAY_REVEAL_CYCLE_FOOTNOTE
                            }
                          />
                        </div>
                      ...
                      {firstDayRevealPhase >= FIRST_DAY_REVEAL_ENERGY ? (
                        <div
