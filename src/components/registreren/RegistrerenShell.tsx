...
type RegistrerenShellProps = {
  ...
  visual?: "work" | "story";
  /** NL/EN in de chrome. Uit op acquisition-signup (minder keuzedruk). */
  showLocaleToggle?: boolean;
  /** Kleiner logo/woordmerk voor rustige signup-viewport. */
...
export function RegistrerenShell({
  ...
  const brandPx = isPlanPage ? 44 : compactBrand ? 40 : 73;
  const brandClass = isPlanPage
    ? "h-11 w-11"
    : compactBrand
      ? "h-10 w-10"
      : "h-[4.55rem] w-[4.55rem]";
      ...
      <div
        className={
          isPlanPage
            ...
            : `mx-auto flex w-full max-w-[25.2rem] flex-col items-center text-center ${compactBrand ? "space-y-4" : "space-y-5"}`
        }
      >
        <div
        ...
        >
          {logoError ? (
            <div
              className={`flex items-center justify-center rounded-2xl shadow-md ${brandClass} ${isStory ? "bg-[var(--story-accent)]" : "bg-blue-600"}`}
            >
              <span
                className={`font-bold text-white ${isPlanPage || compactBrand ? "text-lg" : "text-2xl"}`}
              >
                S
              </span>
            </div>
            ...
            <img
              ...
              width={brandPx}
              height={brandPx}
              className={`object-contain drop-shadow-sm ${brandClass}`}
